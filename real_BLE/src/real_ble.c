...
#define SENSOR_LED_GPIO_NUMBER (12)
...
static void message_port_cb(int local_port_id, const char *remote_app_id, const char *remote_port,bool trusted_remote_port, bundle *message, void *user_data)
{
	...
	dlog_print(DLOG_INFO, LOG_TAG, "message_port_send_message error: %d", ret);
...
Eina_Bool my_timed_cb(void *data)
{
    ...
    dlog_print(DLOG_INFO, LOG_TAG, "Current _section = %d", _section);
   	...
   	local_port_id = message_port_register_local_port("real_ble", message_port_cb, _section);
   	if (local_port_id < 0)
...
bool service_app_create(void *data)
{
	...
	peripheral_gpio_open(SENSOR_LED_GPIO_NUMBER, &gpio_h);
	peripheral_gpio_set_direction(gpio_h, PERIPHERAL_GPIO_DIRECTION_OUT_INITIALLY_LOW);

	peripheral_gpio_write(gpio_h, 1);

	dlog_print(DLOG_INFO, LOG_TAG, "Turn on LED");
...
void service_app_terminate(void *data)
{
	...
	peripheral_gpio_write(gpio_h, 1);
	peripheral_gpio_write(gpio_h, 0);
	return;
}
...
void service_app_control(app_control_h app_control, void *data)
{
	...
	// Set timer
	ecore_timer_add(3.0, my_timed_cb, NULL);
